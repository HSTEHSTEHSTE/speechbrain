...
class SLU(sb.Brain):
    def compute_forward(self, batch, stage):
        ...
        # # dump models
        # tgt_path = '/home/xli257/slu/temp/pre-test/'
        ...
        with torch.no_grad():
            feats = self.hparams.compute_features(wavs)
            feats, wavs, wav_lens = feats.to(self.device), wavs.to(self.device), wav_lens.to(self.device)
            feats = self.modules.normalize(feats, wav_lens)
        ASR_encoder_out = self.modules.enc(feats.detach())
